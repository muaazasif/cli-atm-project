    ...
    if (answer.pin === mypin) {
      ...
      if (operationans.operation === "💸 Withdraw") {
        ...
        if (myBalance >= withdraw.withdrawamt) {
          ...
          console.log(
            chalk.green(
              "💸 Withdrawal successful. Your Remaining Balance is ",
              myBalance
            )
          );
          ...
          console.log(
            chalk.red(
              "💔 Transaction cannot proceed due to insufficient balance. "
            )
          );
        }
        ...
        if (depositAmountResponse.depositAmount >= 0) {
          ...
          console.log(
            chalk.green(
              `💳 ${depositAmountResponse.depositAmount} deposited successfully. Your New Balance is ${myBalance}`
            )
          );
          await askForReceipt(
            depositAmountResponse.depositAmount,
            myBalance,
            "Deposit Money"
          );
        } else {
